(function($) {
	...
	$(function() {
		...
		$('body > ul.dropotron li a').click( function(event) {
			event.preventDefault();
			event.stopPropagation();
			var link = $(this).data('link');	
			if(link) {
		...
		// handlers for mobile skel
		$('#navPanel nav a').click(function(event) {
			var link = $(this).text().toLowerCase().replace(/ /g,'-');
			if(link === "hemstädning" || link === "hotellstädning")
				link = "feature1";
			if(link === "kontorsstädning" || link === "trappstädning")
				link = "feature2";
			if(link === "bemanning")
				link = "feature3";
			if(link === "broschyr-(pdf)"){
				alert('Digital broschyren kommer inom kort');
				return false;
			}
			if(link !== undefined)
				scrollToAnchor(link);
		...
		// deactivate links in images
		$('div.image-wrapper > a.image').click(function(event) {
			event.preventDefault();
		});;
